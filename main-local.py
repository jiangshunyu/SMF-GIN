...
from util import datasub_load_data, sub_segregate
...
def valtest_batch(test_graph, iteration, task, ways, spt_shots, qry_shots, args, retrain=False):
    ...
    for i, j in enumerate(way):
        for m in spt_shot[i]:
            x_spt.append(test_graph[j][m][0])
            x_spt.append(test_graph[j][m][1])
            x_spt.append(test_graph[j][m][2])
        for n in qry_shot[i]:
            x_qry.append(test_graph[j][n][0])
            x_qry.append(test_graph[j][n][1])
            x_qry.append(test_graph[j][n][2])
...
def train_batch(train_graph, iteration, task, ways, spt_shots, qry_shots, args, retrain=False):
    ...
    for i, j in enumerate(way):
        for m in spt_shot[i]:
            x_spt.append(train_graph[j][m][0])
            x_spt.append(train_graph[j][m][1])
            x_spt.append(train_graph[j][m][2])
        for n in qry_shot[i]:
            x_qry.append(train_graph[j][n][0])
            x_qry.append(train_graph[j][n][1])
            x_qry.append(train_graph[j][n][2])
...
def metric_prediction(i, j, gallery, query, train_label, metric_type):
    ...
    if i == 460:
        x = distance.cpu().detach().numpy().astype(np.float32)
        np.save('/home/jsy/SimpleShot/save/T-sub-0.7703/val_%s.npy' % j, x)
...
def train(train_graph, args, iteration, train_way, train_spt_shot, train_qry_shot, model, device, optimizer):
    ...
    for i in range(args.task_num):
        ...
        spt_out1, spt_out2 = model(x_spt)
        qry_out1, qry_out2 = model(x_qry)

        spt_out = model.attention_local(spt_out1, spt_out2)
        qry_out = model.attention_local(qry_out1, qry_out2)


        spt_output = spt_out.reshape(args.num_ways, args.spt_shots, -1).mean(1)
        output = -get_metric(args.meta_train_metric)(spt_output, qry_out)
        ...
        prediction = metric_prediction(1, 1, spt_output, qry_out, y_spt, args.meta_val_metric)
...
def val(val_graph, args, iteration, val_way, val_spt_shot, val_qry_shot, model, device):
    ...
    for i in range(args.val_task_num):
        ...
        spt_output = spt_out.reshape(args.num_ways, args.spt_shots, -1).mean(1)
        y_spt = y_spt[::args.spt_shots]
        prediction = metric_prediction(iteration, i, spt_output, qry_out, y_spt, args.meta_val_metric)
        acc = (prediction == y_qry).float().mean()
...
def test(train_graph, test_graph, args, iteration, test_way, test_spt_shot, test_qry_shot, model, device):
    ...
    for j in range(len(train_graph)):
        x = []
        for i in range(len(train_graph[j])):
            x.append(train_graph[j][i][0])
        y = model(x)
    ...
    for i in range(args.test_task_num):
        ...
        spt_output = spt_out.cpu().detach().numpy().astype(np.float32)
        qry_output = qry_out.cpu().detach().numpy().astype(np.float32)
...
def main():
    ...
    parser.add_argument('--concat_type', type=str, default="mean+", choices=["mean", "mean+"],
    ...
    parser.add_argument('--input_model_file', type=str, default="./data_sub_save/",
    ...
    all_graphs, label_dict = datasub_load_data(args.dataset, True)
    train_graph, test_graph = sub_segregate(args, all_graphs, label_dict)

    # input_dim = 718
    input_dim = train_graph[0][0][0].node_features.shape[1]
    num_classes = args.num_ways

    model = GraphCNN(args, args.num_layers, args.num_mlp_layers, input_dim, args.hidden_dim, num_classes,
                     args.final_dropout,
    ...
    for iteration in range(1, args.iterations + 1):
        ...
        train_loss, train_acc = train(train_graph, args, iteration, train_way, train_spt_shot, train_qry_shot,
                                      model,
                                      device,
                                      optimizer)
        ...
        if iteration % 20 == 0:
            ...
            val_acc, val_conf = val(test_graph, args, iteration, val_way, val_spt_shot, val_qry_shot, model,
                                    device)
            test_acc, test_conf = test(train_graph, test_graph, args, iteration, test_way, test_spt_shot,
                                       test_qry_shot,
                                       model, device,
                                       )
            acc2.append(val_acc)
